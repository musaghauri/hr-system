...
const mongoDriver = new MongoHanlder(Project);
const activeDriver = mongoDriver;

const CRUD = new CrudHandler('Project', activeDriver, mongoDriver);
...
function UpdateById(projectId, projectBody, user = null) {
  return new Promise((resolve, reject) => {
    const { name = null } = projectBody;

    ifAttributeExists({ name }, name, projectId).then(data => {
      if(!!(_get(data, 'status'))) {
        reject({
        ...
        CRUD.updateById(projectId, projectBody, user).then(updatedProject => {
...
function Create(projectBody, user = null) {
  return new Promise((resolve, reject) => {
    const { name = null } = projectBody;
    ...
    ifAttributeExists({ name }, name).then(data => {
      if(!!(_get(data, 'status'))) {
        ...
        CRUD.create(Project, projectBody, user).then(savedProject => {
